...
感觉需要学一波动态调试？我都觉得应该就是这个了，这个明显就是使用`/usr/sbin/apache2 -k start`来隐藏正常的命令执行

现在遇到的问题就是
- 命令是从那里传进去的，的确找到了两个url可能使用post或者get等方法传入了一个参数，或者反弹了shell，总之就是没有找到具体这个后门使用的方法，只是摸到了一个大概
- 输入的命令是怎么在程序里面执行的，只是找到了一个可疑的点

先尝试一下ida动态调试吧，感觉这个进行不太下去了，需要一些逆向知识和关于apache动态链接库的知识

# 动态调试
- 先找到ida安装目录下的`dbgsrv`文件夹，这个目录存放一些远程调试的服务端程序。（感觉这个软件真是有点强）
> 我这里面直接拷贝了一个linux_server64到根目录下，附上777，直接执行
- 打开ida连接服务端程序
> 打开ida，选择菜单栏中的Debugger=> attach =>Remote linux debugger,然后一般填好ip就ok可以了，注意关闭linux里面的防火墙，
- 之后会弹出一个窗口，显示当前Linux下的进程，选择我要调试的`/usr/sbin/apache2 -k start`进程

再然后又是一个看不懂的界面没逆向基础分析这个还是有点难

按照我以前了解的应该是找一个合适的地方设置一个断点，然后单步调试（翻了一天的ida鼠标滚轮都要滚坏了，这时候急需要一个罗技的无线滚轮）

**发现需要访问`http://192.168.72.137/?hackinglabwelcomeyou!`中的来启动这个`"apache2"`的进程,不访问的话这个进程就会消失**


今天又稍微看了一下。感觉还是需要一些逆向知识和apache模块编写的基础才能继续深入。感觉还有可能需要一些进程间通信的基础，好想知道这个题是怎么出的啊啊啊啊啊。
...
- [Apache Server Project](https://httpd.apache.org/docs/2.4/developer/modguide.html)
